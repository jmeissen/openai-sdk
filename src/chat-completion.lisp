...
(defclass file (openai-request)
  ((file-data :initarg :file-data
              :accessor file-data
              :type string
              :documentation "The base64 encoded file data, used when passing the file to the model as a stringThe base64 encoded file data, used when passing the file to the model as a string.")
   ...
   (filename :initarg :filename
             :accessor filename
             :type string
             :documentation "The name of the file, used when passing the file to the model as a string.")))
...
(defclass function-call (openai-request)
  ((arguments :type string
              :accessor arguments
              :initarg :arguments
              :documentation "he arguments to call the function with, as generated by
...
(defun make-function-call (name arguments)
  (make-instance 'function-call :name name :arguments arguments))
...
(defun make-chat-completion (model messages &rest args &key audio frequency-penalty function-call
                                                         functions logit-bias logprobs
                                                         max-completion-tokens max-tokens
                                                         metadata modalities n parallel-tool-calls
  ...
  (apply #'make-instance 'chat-completion :messages messages :model model args))
